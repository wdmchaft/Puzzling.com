...
def change_password(name, password):
  r = requests.put(root + "/login", data={"name" : name, "password" : password})
  return json.loads(r.content)

# DELETE USER
def delete_user(name):
  r = requests.delete(root + "/login", data={"name" : name})
  return json.loads(r.content)
...
def basic_test():
  ...
  assert(create_user("johndoe", "nobody")["username"] == "johndoe"), "Couldn't create user"
  ...
  assert(change_password("johndoe", "somebody")["status"] == "SUCCESS"), "Couldn't change password"
  ...
  print "Success: Password checked"
  assert(delete_user("johndoe")["status"] == "SUCCESS"), "Couldn't delete user"
...
def error_test():
  ...
  assert(create_user("a", "a")["username"] == "a"), "Couldn't create user"
  print "Success: Created user"
  ...
  assert(change_password("a", "asdf")["status"] == "SUCCESS"), "Couldn't change password"
  print "Success: Changed password"
  ...
  assert(not "authtoken" in change_password("b", "asfas")), "Was able to change password of non-existent user"
  print "Success: Can't change password of non-existent user"
  assert(not "status" in delete_user("b")), "Was able to delete non-existent user"
  ...
  assert(delete_user("a")["status"] == "SUCCESS"), "Wasn't able to delete user"
